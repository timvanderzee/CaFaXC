...
[fullpath, name, ext] = fileparts(which('calf_parms.m')); % folder contains the present m-file
cd(fullpath)
cd ..
addpath(genpath(cd))
...
parms.ce.lce0 = .07; % [m], CE length at phi = 0, determines position torque-angle relation
parms.mtc.lmtc0 = parms.see.lse0 + parms.ce.lce0;  % MTC length at phi = 0

%% evaluate torque-angle relation
fl = cfxc.evaluate_force_length((-30:2.5:30), parms, show_figs);
...
parms.exp.phi = 0; % joint angle (full extension = 0)
...
if save_parms
    cd(fullpath)
    save('calf_parms.mat','parms','fv','fl')
    disp('Saved: calf_parms.mat')
end
